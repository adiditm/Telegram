...
$vScriptName = $_SERVER['SCRIPT_NAME'];

$vScriptName=explode("/",$vScriptName);

//print_r($_POST);

$vScriptName=$vScriptName[count($vScriptName)-1];
...

{
	...
	return true;
		...
		echo '<form onsubmit="return formSubmit()" enctype="multipart/form-data"  class="',$this->getCSSclass('form'),'" method="post"';

			//echo ' action="',$page_name,'" name="'.$this->cgi['prefix']['sys'].'form">',"\n"; //asli

			echo ' action="',"",'" name="'.$this->cgi['prefix']['sys'].'form">',"\n"; //asli
...

	{
		...
		for ($i = ($this->tabs[0] == 'PMEtab0' ? 1 : 0); $i < count($this->tabs); $i++) {
			...
			echo '<td align="left" class="',$css_class_name,'" id="'.$this->dhtml['prefix'].'tab',$i,'_',$position,'_label">';
		...
		echo '<td align="left"  class="',$this->getCSSclass('tab-end', $position),'">&nbsp;</td>',"\n";
...

	{
		...
		for ($tab = 0, $k = 0; $k < $this->num_fds; $k++) {
			...
			echo '<td align="left" class="',$this->getCSSclass('key', null, true, $css_postfix),'">';

			echo $this->fdd[$k]['name'],'</td>',"\n";

			echo '<td align="left"  class="',$this->getCSSclass('value', null, true, $css_postfix),'"';
			...
			if ($this->col_has_values($k)) {
				...
				//echo '<div class="col-md-6"><textarea class="form-control ',$css_class_name,'" name="',$this->cgi['prefix']['data'].$this->fds[$k],'"';

				echo '<div class="col-md-6"><textarea class="form-control ',$css_class_name,'" name="',$this->cgi['prefix']['data'].$this->fds[$k],'" id="',$this->cgi['prefix']['data'].$this->fds[$k],'"'; //didit
				...
				echo '<div class="col-md-6"><input class="form-control ',$css_class_name,'" ';

								if ($this->fdd[$k]['select'] == 'FL')

				   echo ($this->password($k) ? 'type="password"' : 'type="file"');    

				else

				   echo ($this->password($k) ? 'type="password"' : 'type="text" '); //didit
				...
				echo ' name="',$this->cgi['prefix']['data'].$this->fds[$k],'" id="',$this->cgi['prefix']['data'].$this->fds[$k],'"'; //didit
			...
			if ($this->guidance) {
				...
				echo '<td align="left" class="',$css_class_name,'">',$cell_value,'</td>',"\n";
...

	{
		...
		if ($this->col_has_values($k)) {
			...
			$vNama=$this->cgi['prefix']['data'].$this->fds[$k];

			if (preg_match("/fharga/i",$vNama,$vMatches) || preg_match("/fpoin/i",$vNama,$vMatches)  || preg_match("/fberat/i",$vNama,$vMatches) || preg_match("/fratenom/i",$vNama,$vMatches) || preg_match("/funit/i",$vNama,$vMatches)) {
	...
	//by Didit
		...
		//	   echo '<input class="',$css_class_name,'" type="file"';
echo '<div style="display:inline" class="col-md-6 col-sm-6 col-xs-4"><input  class="form-control ',$css_class_name,'" type="file" '.$thedir;
			else

			//   echo '<input class="',$css_class_name,'" type="text" ';
			   
			echo '<div style="display:inline" class="col-md-6 col-sm-6 col-xs-4"><input  class="form-control ',$css_class_name,'" type="text" '.$thedir;

			echo ($this->readonly($k) ? ' disabled' : '');

			//echo ' name="',$this->cgi['prefix']['data'].$this->fds[$k],'" value="';

			echo ' name="',$this->cgi['prefix']['data'].$this->fds[$k],'" id="',$this->cgi['prefix']['data'].$this->fds[$k],'" value="';//didit

			if($escape) echo htmlspecialchars($row["qf$k"]);

			else echo $row["qf$k"];

			if ($this->fdd[$k]['select'] == 'FL' && trim($this->cellDisplay($k, $row, $css_class_name)) != "&nbsp;") 

			   $gamb="&nbsp;<IMG border='1' style='max-width:200px'  SRC=\"../images/user/".$this->cellDisplay($k, $row, $css_class_name)."\">";
...

	{
		...
		echo '<td align="left" class="',$css_class_name,'"',$this->getColAttributes($k),">\n";
...

	{
        ...
        if ($label !='Cancel')
		    ...
		    $ret = '<input'.$markdisabled.' type="submit" class="btn btn-success btn-sm '.$css_class_name
		    ...
		    $ret = '<input'.$markdisabled.' type="submit" class="btn btn-default btn-sm '.$css_class_name
...

	{
		...
		if ($css=='pme-goto') {
			...
			$sty='style="height:39px;margin-top:+2px"';

		} else { 

		    

			$otherclass='form-control';
		...
		$vAllName = array('PME_data_fidcat','PME_data_faktif','PME_data_fcountry_code');

		if (in_array($name,$vAllName)) {

		   $vCss='style="width:195px"';

		   $vClassAdd=' form-control ';   
		...
		$ret.= '<select '.$vMulti.' '.$vCss.' '.$sty.'  class="'.$vClassAdd.' '.$otherclass.' '.htmlspecialchars($css).'" id="'.htmlspecialchars($name).'" name="'.htmlspecialchars($name);
...

	{
		...
		echo '<div class="form-inline"><td align="left" class="',$this->getCSSclass('buttons', $position),'">',"\n"; //by didit
		...
		if($this->display['num_pages'] || $this->display['num_records'])
			...
			echo '<td  align="left" class="',$this->getCSSclass('stats', $position),'">',"\n";
		...
		if($this->display['num_pages'] || $this->display['num_records']) echo '</td></div>';//by didit
...

	{
		...
		echo '<td  align="left" class="',$this->getCSSclass('buttons', $position),'">',"\n";
		...
		if (strlen(@$this->message) > 0) {
			...
			echo '<td align="left" class="',$this->getCSSclass('message', $position),'">',$this->message,'</td>',"\n";
...

	{
		...
		echo '<div class="table-responsive"><table class="table table-striped  table-hover table-border ',$this->getCSSclass('main'),'" summary="',$this->tb,'">',"\n";
		...
		if ($this->filter_operation()) {
			...
			if ($fields != false) {
				...
				echo '<td align="left" class="',$css_class_name,'" colspan="',$sys_cols,'">';
				...
				for ($k = 0; $k < $this->num_fds; $k++) {
					...
					echo '<td align="left" class="',$css_class_name,'">';
					...
					if ($this->password($k)) {
						...
						echo '" type="text" name="'.$this->cgi['prefix']['sys'].'qf'.$k.'" id="'.$this->cgi['prefix']['sys'].'qf'.$k.'"',$len_props; //didit
		...
		while ((!$fetched && ($row = $this->sql_fetch($res)) != false)
			...
			if ($sys_cols) { /* {{{ */
				...
				if ($select_recs) {
					...
					if ($this->nav_buttons()) {
						echo '<input style="display:none" type="radio" name="mychecked" value="'.htmlspecialchars($key_rec).'" id="'.htmlspecialchars($key_rec).'"  >';

						echo '<td class="',$css_class_name,'"><input  style=";" class="',$css_class_name;
						echo '" type="radio" onclick="document.getElementById(\''.htmlspecialchars($key_rec).'\').checked=true;" name="'.$this->cgi['prefix']['sys'].'rec';

						//echo '" type="radio" tabindex="3" name="'.$this->cgi['prefix']['sys'].'rec';
			...
			for ($k = 0; $k < $this->num_fds; $k++) { /* {{{ */
				...
				echo '<td align="left" class="',$css_class_name,'"',$this->getColAttributes($fd),'>';
...

	{
		...
		if  (preg_match("/,`fimaget`=''/i",$query_real)) {

		    $query_real = str_replace(",`fimaget`=''","",$query_real);

			

		} else if  (preg_match("/,`fimage`=''/i",$query_real)) {

		    $query_real = str_replace(",`fimage`=''","",$query_real);
