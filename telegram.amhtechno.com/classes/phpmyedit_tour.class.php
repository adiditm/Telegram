...

{
...

	{
		...
		for ($tab = 0, $k = 0; $k < $this->num_fds; $k++) {
			...
			if ($this->col_has_values($k)) {
				...
				} else { //add didit + last param as eattr

					echo $this->htmlSelect($this->cgi['prefix']['data'].$this->fds[$k],

							$css_class_name, $vals, $selected, $multiple, $readonly,

							$strip_tags, $escape,'',$this->fdd[$k]['select']);

				}

			} elseif (isset ($this->fdd[$k]['textarea'])) {

				echo '<textarea  style="overflow-y: scroll;overflow-x: hidden;" class="',$css_class_name,'" name="',$this->cgi['prefix']['data'].$this->fds[$k],'"';
...

	{
		...
		if ($this->col_has_values($k)) {
			...
			} else { //change didit + last param as eattr
...
/*				echo $this->htmlSelect($this->cgi['prefix']['data'].$this->fds[$k],
			...
			if ($this->fdd[$k]['spec'] == 'Y' && $this->fdd[$k]['input|C'] == 'R')

			   $ket='<table><tr><td><blink><font color="#FF000"> Gantilah Kode Barang dan editlah data lain yang diperlukan</font></blink></td></tr></table>';
...

	{
		...
		if ($this->fdd[$k]['select'] == 'FL' && trim($this->cellDisplay($k, $row, $css_class_name))!='&nbsp;') //didit

		   echo "&nbsp;<IMG border='1' style='max-width:200px' SRC=\"images/user/".$this->cellDisplay($k, $row, $css_class_name)."\">";
...

	{
		...
		if ($escape) {
			...
			//$value = htmlspecialchars($value); Didit Google maps
	...
	function htmlSelect($name, $css, $kv_array, $selected = null, /* ...) {{{ */
			...
			/* booleans: */ $multiple = false, $readonly = false, $strip_tags = false, $escape = true, $js = NULL,$eattr='')

	{
		...
		if ($eattr=='EA1') 
		   ...
		   $vID='id="'.htmlspecialchars($name).'" onChange=changeSubjen(this.value) ';

        if ($eattr=='EA2')

		   $vID='id="'.htmlspecialchars($name).'" onChange=changeObjek(this.value) ';

        if ($eattr=='EA3')

		   $vID='id="'.htmlspecialchars($name).'"';

		 //css by didit

		$ret = '<div style="display:inline" id="div'.htmlspecialchars($name).'"><select '.$vID.' class="'.htmlspecialchars($css).'" name="'.htmlspecialchars($name);
		...
		foreach ($kv_array as $key => $value) {
			...
			if ($eattr=='EA1')
			   ...
			   $vEvent='onClick="changeSubjen(this.value)"';

			

			$ret .= '<option '.$vEvent.' value="'.htmlspecialchars($key).'"';
			...
			if ($sys_cols) { /* {{{ */
				...
				if ($select_recs) {
					...
					if ($this->nav_buttons()) {
						...
						echo '" value="',htmlspecialchars($key_rec),'" id='.htmlspecialchars($key_rec); //didit htmlspecialchars($key)
