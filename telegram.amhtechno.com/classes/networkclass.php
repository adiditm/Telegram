<?php
  // include_once "../config.php";   
   ...
   include_once CLASS_DIR."espayclass.php";  
...
// di sini
   ...
   // $vBelanja = $oJual->getKITNom($vSernoMem);
	$vBelanja = $oRules->getSettingByField('fhrgjoin');
    $vSponFee = $vBelanja * $vProsenFeeSpon /100;
	// $vSponFee = $oRules->getSettingByField('fprosenspon') * 1800000 / 100;
	  $vPresFee = $vBelanja * $oRules->getSettingByField('fpresfee') / 100;
...
/*
   ...
   //Masukkan komisi matrix
   function sendFeeTitikCompress($pID,$pDeep,$pMulti=1,$pRef,$vDateCompile,$pDateBonus) {
...
//Level 0
				...
				echo $vMsg="<br><font color='#f0f'>Insert fee titik $pID --> $pID (level 0) dari RO $pRef  sebesar ($pMulti x $vFeeSet0"."%) =  $vTitikFee0 !</font>";
						 $vMsgAll .= $vMsg;
				$vsql="insert into tb_kom_mtx(fidregistrar,ftanggal,fidfunder,fidmember,ffee,ffeestatus,fdesc) values('$pID','$pDateBonus','$pID','$pID',$vTitikFee0,'1','$pRef')";   
				 $oDB->query($vsql);
...
//echo "$vIncomeMonth > $vPTKPMonth || $vIncomeYear > $vPTKPYear";
				if ($vIncomeMonth > $vPTKPMonth || $vIncomeYear > $vPTKPYear) {
				 ...
				 $oKomisi->insertMutasi($pID,$vFunder,$pDateBonus,$vDesc,$vTitikFeeNett,0,$vNewBal,'unile',$pRef);
...
//End level 0				 
	...
	//echo "$i Ada<br>";
		  ...
		  if ($vIDReceiver!=-1 && $vIDReceiver!='-' ) {
				...
				$vsql="insert into tb_kom_mtx(fidregistrar,ftanggal,fidfunder,fidmember,ffee,ffeestatus,fdesc) values('$pID','$pDateBonus','$vFunder','$vIDReceiver',$vTitikFee,'1','$pRef')";   
				 $oDB->query($vsql);
			//	 echo $vsql."<br>";
...
//echo "$vIncomeMonth > $vPTKPMonth || $vIncomeYear > $vPTKPYear";
				if ($vIncomeMonth > $vPTKPMonth || $vIncomeYear > $vPTKPYear) {
				 ...
				 $oKomisi->insertMutasi($vIDReceiver,$vFunder,$pDateBonus,$vDesc,$vTitikFeeNett,0,$vNewBal,'unile',$pRef);
...
echo $vMsg="<br><font color='#f0f'>Insert fee titik $pID --> $vIDReceiver (no level) dari RO $pRef  sebesar 0 (compressed) !</font>";
			  ...
			  if ($vIDReceiver!=-1 && $vIDReceiver!='-' ) {
					$vsql="insert into tb_kom_mtx(fidregistrar,ftanggal,fidfunder,fidmember,ffee,ffeestatus,fdesc) values('$pID','$pDateBonus','$vFunder','$vIDReceiver',0,'0','$pRef')";   
					 $oDB->query($vsql);			
			  }
