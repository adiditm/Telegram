...
$vsql="select * from m_korwil where flevel='SUBKOR' ";

$vsql.=$vCrit;
$vsql.=" order by fidkorwil ";
...
function MM_goToURL() { //v3.0
  var vChecked=$('.classRB:checked').length;
  if (parseInt(vChecked) < 1 ) {
	alert('Pilih subkorwil terlebih dahulu!');  
	return false;
  }
  var i, args=MM_goToURL.arguments; document.MM_returnValue = false;
  if (getValue()=="") {
...
function doDel(pParam,pIDTr) {
   ...
   if (confirm('Are you sure to delete this subkorwil ('+pParam+')')) {
	  ...
	  $.get(vURL,function(data){
		  ...
		  if (data=='success') {
			...
			alert('Subkorwil '+pParam+' already deleted!')  ;
...
*/
	...
	<div class="right_col" role="main">
		...
		<div><label>
		<h3>Subkorwil Profile</h3></label></div>
...
<div align="left" class="table-responsive col-lg-8" style="min-height:270px" > 
		  ...
		  <table border="0" cellpadding="4" cellspacing="0"  align="left" >
            ...
            <tr>
              <td height="25" style="width: 14%" ><div align="left">ID Korwil </div></td>
...
<div class="row" style="text-align:left">
  ...
  <? if ($vCurrent=='mdm_master_data') { ?>      
       <? if (true || $oSystem->checkPriv($vUser,"mdm_listprof_detail")) { ?> 
       <input alt="Detail / Verifikasi Data" name="btDAddKor" type="button" class="btn btn-success btn-sm" id="btDAddKor" onClick="document.location.href='../manager/addsubkorwil.php?op=&current=mdm_master_data&'" value="Tambah Sub Korwil &raquo;" onMouseovers="showhint('Lihat detail /  Edit Member '+getValue(), this, event, '220px')" style="margin-top:5px" >  <? } ?> 
       
       <? } ?>
       
  <? if ($vCurrent=='mdm_master_data') { ?>      
          <? if (true || $oSystem->checkPriv($vUser,"mdm_listprof_genealogi")) { ?> <input alt="Detail / Edit Subkorwil" name="btDetail" type="button" class="btn btn-success btn-sm" id="btDetail" onClick="return MM_goToURL('parent','../manager/regkorwil.php?current=<?=$vCurrent?>&op=<?=$vSpy?>'+CryptoJS.MD5(getValue().trim())+'&uMemberId='+getValue()+'&j=sub');return document.MM_returnValue" value="Detail / Edit SubKorwil &raquo;" onMouseovers="showhint('Lihat detail /  Edit Member '+getValue(), this, event, '220px')" style="margin-top:5px" >  <? }  ?>
		  
		  
        <? if (true || $oSystem->checkPriv($vUser,"mdm_listprof_genealogi")) { ?> 
        
        <input alt="Area" name="btDetail" type="button" class="btn btn-success btn-sm" id="btDetail" onClick="return MM_goToURL('parent','../masterdata/mdet_template.php?current=<?=$vCurrent?>&op=<?=$vSpy?>'+CryptoJS.MD5(getValue().trim())+'&uMemberId='+getValue()+'&j=sub');return document.MM_returnValue" value="Area Subkorwil &raquo;" onMouseovers="showhint('Lihat detail /  Edit Member '+getValue(), this, event, '220px')" style="margin-top:5px" >  <? } 
 ...
 <? if($vAktifList=='1') { ?>	
        ...
        <td class="hide"  style="height: 39px;<? if ($db->f('fpriv')=='1') echo 'background-color:#ccc';?>"><div align="right">
          ...
          <?=number_format($vSaldoAcc,0,",",".")?>
              ...
              <td  style="height: 39px" <? if($vFall==$db->f('fidkorwil')) echo "bgcolor=#0f0";?>><span >

                

                <input class="classRB" id="rbSelected<?=$db->f('fidkorwil')?>" style="width:20px;height:20px" name="rbSelected" type="radio" value="<?=$db->f('fidkorwil')?>" onClick="checkStatus('<?=$db->f('faktif')?>','<?=$db->f('fstockist')?>')" <? if ($vChoosed==$db->f('fidkorwil')) echo 'checked'; ?> >
...
<? if ($vCurrent=='mdm_master_data') { ?>      
       <? if (true || $oSystem->checkPriv($vUser,"mdm_listprof_detail")) { ?> <input alt="Detail / Verifikasi Data" name="btDAddKor1" type="button" class="btn btn-success btn-sm" id="btDAddKor1" onClick="document.location.href='../manager/addsubkorwil.php?op=&current=mdm_master_data&'" value="Tambah Sub Korwil &raquo;" onMouseovers="showhint('Lihat detail /  Edit Member '+getValue(), this, event, '220px')" style="margin-top:5px" >  <? } ?> 
       <? } ?>

 <? if ($vCurrent=='mdm_master_data') { ?>      
       <? if (true || $oSystem->checkPriv($vUser,"mdm_listprof_detail")) { ?> <input alt="Detail / Edit Subkorwil" name="btDetail3" type="button" class="btn btn-success btn-sm" id="btDetail3" onClick="return MM_goToURL('parent','../manager/regkorwil.php?current=<?=$vCurrent?>&op=<?=$vSpy?>'+CryptoJS.MD5(getValue().trim())+'&uMemberId='+getValue()+'&j=sub');return document.MM_returnValue" value="Detail / Edit Subkorwil &raquo;" onMouseovers="showhint('Lihat detail /  Edit Member '+getValue(), this, event, '220px')" style="margin-top:5px" >  <? } ?>        
         ...
         <!--  <? if (true || $oSystem->checkPriv($vUser,"mdm_listprof_repkit")) { ?><input name="btKit" type="button" class="btn btn-success btn-sm" id="bKit" onClick="return MM_goToURL('parent','../memstock/rptkitphp?current=<?=$vCurrent?>&op=<?=$vSpy?>'+CryptoJS.MD5(getValue().trim())+'&uMemberId='+getValue());return document.MM_returnValue" value="Agent KIT Report &raquo;" onMouseovers="showhint('KIT Agent '+getValue(), this, event, '210px')" style="margin-top:5px" /> <? } ?> -->
          ...
          <? if (true || $oSystem->checkPriv($vUser,"mdm_listprof_uwallet")) { ?><input name="btWallet" type="button" class="btn btn-success btn-sm" id="btWallet" onClick="return MM_goToURL('parent','../masterdata/mdet_template.php?current=<?=$vCurrent?>&op=<?=$vSpy?>'+CryptoJS.MD5(getValue().trim())+'&uMemberId='+getValue()+'&j=sub');return document.MM_returnValue" value="Area Subkorwil &raquo;" onMouseovers="showhint('Nex-Wallet '+getValue(), this, event, '210px')" style="margin-top:5px" /> <? }
 }
...
<ul class="pagination" >
   ...
   for ($i=0;$i<$vPageCount;$i++) {
     ...
     if ($idisp!=$vPage) {
?>
              <li ><a  href="aktif.php?lmAktif=<?=$vAktif?>&lmMship=<?=$vPrem?>&uPage=<?=$idisp?>&lmSort=<?=$vSort?>&current=<?=$_REQUEST['current']?>" >
              <?=$i+1?>
              </a> </li> 
              <?
  } else {
