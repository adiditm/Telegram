  ...
  // print_r($_POST);
   ...
   //Purchase
    ...
    while (list($key,$val) = @each($_SESSION['savestock'])) {
    	...
    	$vSQL="insert into tb_trxstok(fidpenjualan,fidseller, fidmember, falamatkrm, fnostockist, fidproduk, fjumlah, ftanggal, fhargasat, fsubtotal, fsize, fcolor, ftgltrans, fjenis, fjmltrans, fserial, fpin, fmethod, fketerangan, ftglentry, fprocessed, ftglprocessed,fkindtrx)";
     	...
     	$vSQL.=" values('$vNextJual','$vSeller','$vBuyer','$vAlamat','$vSeller','".$val['lmKode']."',".$val['txtJml'].",now(),".$val['hHarga'].",".$val['hSubTot'].",'".$val['lmSize']."','".$val['lmColor']."',now(),'PS',0,'','','mTrans','Stockist Order',now(),'2','1981-01-01 00:00:00','purcn')";

  	 	$db->query($vSQL);



		//tambah stock stockist
		...
		$vSQL .="values('$vBuyer','".$val['lmKode']."','".$val['lmSize']."','".$val['lmColor']."','$vSeller',now(), 'Pembelian PO dari $vSeller/Admin',".$val['txtJml'].",0 ,$vNewBal,'BFO','1' , '$vUser',now(), '$vNextJual');";//belum selesai

		$db->query($vSQL);

		

		//kurangi stock kantor

		$vLastBal = $oMember->getStockPos($vSeller,$val['lmKode'],$val['lmSize'],$val['lmColor']);

		$vNewBal=(float) $vLastBal - (float) $val['txtJml'];

		$vSQL="insert into tb_mutasi_stok(fidmember, fidproduk,fsize,fcolor, fidfunder, ftanggal, fdesc, fcredit, fdebit, fbalance, fkind, fstatus, flastuser, flastupdate, fref) ";	  	 	

		$vSQL .="values('$vSeller','".$val['lmKode']."','".$val['lmSize']."','".$val['lmColor']."','$vUser',now(), 'Pembelian PO dari $vSeller/Admin',0,".$val['txtJml']." ,$vNewBal,'BFO','1' , '$vUser',now(), '$vNextJual');";//belum selesai
		...
		if ($dbin->num_rows() < 1) {
  	 	...
  	 	//Kurangi stock position Warehouse

		$vSQLx="update tb_stok_position set fbalance=fbalance+".$val['txtJml']." where fidmember='$vSeller' and fidproduk='".$val['lmKode']."' and fsize='".$val['lmSize']."' and fcolor='".$val['lmColor']."';";
   ...
   if (count($_POST['KIT_E']) >=1)
    ...
    while (list($key,$val) = each($_POST['KIT_E'])) {
    	...
    	$vSQL.=" values('$vNextJual','$vSeller','$vBuyer','$vAlamat','$vUser','".$_POST['KIT_E'][$key]."',1,now(),$vHrgKit,$vSubTot,'ROK','mTrans','First KIT Order',now(),'2','1981-01-01 00:00:00','kit1')";
 ...
 belum selesai */
   ...
   if (count($_POST['KIT_B']) >=1)
    ...
    while (list($key,$val) = each($_POST['KIT_B'])) {
    	...
    	$vSQL.=" values('$vNextJual','$vSeller','$vBuyer','$vAlamat','$vSeller','".$_POST['KIT_B'][$key]."',1,now(),$vHrgKit,$vSubTot,'ROK','mTrans','First KIT Order',now(),'2','1981-01-01 00:00:00','kit1')";
   ...
   if (count($_POST['KIT_F']) >=1)
    ...
    while (list($key,$val) = each($_POST['KIT_F'])) {
    	...
    	$vSQL.=" values('$vNextJual','$vSeller','$vBuyer','$vAlamat','$vSeller','".$_POST['KIT_F'][$key]."',1,now(),$vHrgKit,$vSubTot,'ROK','mTrans','First KIT Order',now(),'2','1981-01-01 00:00:00','kit1')";
									...
									<div class="panel-body">
										...
										<div class="">
											...
											<div class="iconic-input">
												...
												<input type="text" class="form-control" id="tfSernoSpon" name="tfSernoSpon" placeholder="Username*" onBlur="checkKitStockist(this)" onKeyUp="setUpper(this)">
										...
										<div class="form-group" style="margin-left:-15px" id="phonemailspon">
											...
											<div class="col-lg-6 col-md-6 divtr" >
												...
												<span style="font-weight:bold">Mailing Address</span></label>
...
<div class="panel panel-default" id="panelkanan">
					                    ...
					                    <div class="panel-heading" >
					                             ...
					                             <div class="panel-title" style="margin-top:-10px">
					        						...
					        						&nbsp;&nbsp;<!-- | &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" id="cbNoPurc" name="cbNoPurc" onclick="setBelanja(this)" value="1" /><span style="color:yellow"> 
													 No Purchase</span> -->
...
<table class="table" style="table-layout:fixed;">
                            ...
                            <tr>
                                ...
                                <th width="9%">Item Qty</th>

                                <th width="9%">Set Qty</th>

                                <th width="11%">Set Price</th>
                            ...
                            <tr id="trAdd" style="display:">
                                ...
                                <td style="display:none; height: 30px;"></th><input type="hidden" name="hSubTot" id="hSubTot" value="" /></th>
...
<div style="font-size:18px;font-weight:bold">Total : <span id="totBelanja">300.000</span></div>
