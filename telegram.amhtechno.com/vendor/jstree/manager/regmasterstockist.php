...
//echo "SAVESTOCK";  
 //print_r($_SESSION['savestock']);
  ...
  // print_r($_POST);
   ...
   $vMinBelanja = $oRules->getSettingByField('fminregmstockist');
   ...
   if ($_POST['hPost'] != '1') {
    ...
    mail($vBCC,"Entri MS Inizialitation Uneeds by $vUser","Display tidak masuk database, hanya sbg debug\n\n\n".print_r($_POST,true)."\n\n\n".print_r($_SESSION['savestock'],true));
   ...
   //Purchase
    while (list($key,$val) = @each($_SESSION['savestock'])) {
		...
		$vSQL .="values('$vBuyer','".$val['lmKode']."','".$val['lmSize']."','".$val['lmColor']."','$vUser',now(), 'Pembelian FO dari $vUser/Admin',".$val['txtJml'].",0 ,$vNewBal,'MBFO','1' , '$vUser',now(), '$vNextJual');";//belum selesai
		$db->query($vSQL);
   ...
   if (count($_POST['KIT_F']) >=1)
    while (list($key,$val) = each($_POST['KIT_F'])) {
    	...
    	$vSQL.=" values('$vNextJual','$vUser','$vBuyer','$vAlamat','$vUser','".$_POST['KIT_F'][$key]."',1,now(),$vHrgKit,$vSubTot,'FK','mTrans','First KIT Order',now(),'2','1981-01-01 00:00:00','kit1')";
  	    ...
  	    $db->query($vSQL);	
   	 	...
   	 	$vSQL="update m_anggota set fstockist =2 where fidmember='$vBuyer'";
  	    $db->query($vSQL);	
    ...
    $db->query('COMMIT;');
     ...
     $oSystem->jsLocation("../manager/regmasterstockist.php");
   }   
...
$(document).ready(function(){
 //  alert('ssss');
  // alert($('#hHarga').val());
   $('#caption').html('Register New Master Agent');
...
function checkKitSpon(pParam) {
   ...
   //alert(vStockist);
      if (parseInt(vStockist)==1) {
         $('#statKitSpon').html('<font color="#f00">'+pParam.value+' : Member Tidak Valid, sudah menjadi agent, harus diupgrade jika diinginkan!</font>');
         document.getElementById('btnSubmit').disabled=true;

     } else if (parseInt(vStockist)==2) {
         $('#statKitSpon').html('<font color="#f00">'+pParam.value+' : Member Tidak Valid, sudah menjadi master agent!</font>');
         document.getElementById('btnSubmit').disabled=true;
			                    ...
			                    <div class="panel panel-default" id="panelkanan">
									<div class="panel-heading" >
										<div class="panel-title" style="margin-top:-10px">
											...
											Data Member Kandidat 
											Master Stockist (Min. Pembelanjaan <?=number_format($vMinBelanja,0,",",".")?>)</label>
											<br style="display: block;margin: -5px 0;" />
											<label for="exampleInputEmail1" style="font-size:12px;">
											Master Agent Candidate </label>
										</div>
									</div>
									<div class="panel-body">
										<div class="">
											...
											<div class="iconic-input">
												...
												<input type="text" class="form-control" id="tfSernoSpon" name="tfSernoSpon" placeholder="Member Card/Starter Kit No*" onBlur="checkKitSpon(this)" onKeyUp="setUpper(this)">
											</div>
										</div>
										<div class="divtr">
