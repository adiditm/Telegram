<? 
	...
	$vMonday = strtotime("+1 day",$vNow);
	$vSunday = strtotime("-1 day",$vNow);	
	//$start_week = strtotime("last monday midnight",$d);
	$vStartWeek=strtotime('last monday', $vMonday);
	$vEndWeek = strtotime("next sunday",$vSunday);
	$vNextEndWeek=strtotime("+1 day",$vEndWeek);
...
if ($vAkhir=="") {
	...
	if (date("Y-m-d",$vNow) <= date("Y-m-d",$vStartWeek))
	   $vAkhir=date("Y-m-d",$vStartWeek);
	else   
	   $vAkhir=date("Y-m-d",$vNextEndWeek);
}
...
if ($vSort =='M')
   ...
   $vOrder.=" order by c.fnamabank  ";
   
else    $vOrder.=" order by a.ftanggal asc ";



			 $vsql="select a.*, b.*, c.fnamabank as cfnamabank, date(a.ftanggal) as ftglcompile from tb_mutasi a left join m_anggota b on a.fidmember=b.fidmember left join m_bank c on b.fnamabank=c.fkodebank where a.fkind='resetweek'"; 
...
$(document).ready(function(){
    $('#caption').html('Weekly Statement Report');
...
<form action="" method="post" name="demoform">
		  ...
		  <table style="width: 100%">
			  <tr>
				  ...
				  <td width="567" style="height: 22px"> <span class="hide"><input style="width:100px;display:inline" class="form-control" name="dc" id="dc" size="11" value="<?=$vAwal?>"><b>to</b></span>
          <input style="width:100px;display:inline" class="form-control" name="dc1" id="dc1" size="11" value="<?=$vAkhir?>"> </td>
			  </tr>
			  <tr class="hide">
				  <td style="width: 90px; height: 26px;" nowrap="nowrap"><strong>Reg. Package</strong>
</td>
...
<tr>
				  <td style="width: 90px; height: 22px" nowrap="nowrap"><strong>Bank Filter</strong>
</td>
			  ...
			  <tr>
				  <td style="width: 90px; height: 22px"><strong>Sort By</strong>
</td>
				  <td style="height: 22px">
...
<? if ($_SESSION['Priv'] == 'administrator') {?>		  
 <tr>
				  <td style="width: 90px; height: 22px"><strong>ID Member</strong>
					</td>
				  <td style="height: 22px">
   ...
   for ($i=0;$i<$vPageCount;$i++) {
     ...
     if ($i!=$vPage) {
?>
          <a href="stateweek.php?uPage=<?=$i?>&amp;uAwal=<?=$oPhpdate->DMY2YMD($oPhpdate->YMD2DMY($vAwal,"-"),"-")?>&amp;uAkhir=<?=$oPhpdate->DMY2YMD($oPhpdate->YMD2DMY($vAkhir,"-"),"-")?>" >
          <?=$i+1?>
          </a>
          <?
  } else {
...
<button class="btn btn-info btn-sm" onClick="document.location.href='../manager/getexcel.php?arr=stateweek&file=weekly_statement_report_<?=$vUser?>'"><i class="fa fa-file-text-o"></i> Export Excel</button> 
