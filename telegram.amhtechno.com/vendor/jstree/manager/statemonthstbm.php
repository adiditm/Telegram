...
if ($vSort =='M')
   ...
   $vOrder.=" order by a.fdebit  ";
			 ...
			 $vsql="select a.*, b.*,c.fnamabank as cfnamabank,  date(a.ftanggal) as ftglcompile from tb_cftrans_smost a left join m_anggota b on a.fidmember=b.fidmember left join m_bank c on b.fnamabank=c.fkodebank where a.ftype='cfsmonth'"; 
			...
			//Export Excel
			   ...
			   $vEmail=$db->f('femail');
				...
				$_SESSION['statemonthstbm']=$vArrData;
...
<div class="right_col" role="main">
		<div><label><h3>Monthly Transfer Bonus Stockist (Bank Mandiri)</h3></label></div>
...
<button class="btn btn-info btn-sm" onClick="document.location.href='../manager/getexcel.php?arr=statemonthstbm&file=monthly_statement_reportst_<?=$vUser?>'"><i class="fa fa-file-text-o"></i> Export Excel</button> &nbsp;
<button class="btn btn-info btn-sm" onClick="document.location.href='../manager/getcsv.php?arr=statemonthstbm&file=daily_statement_reportst_<?=$vUser?>'"><i class="fa fa-file-text-o"></i> Export CSV</button> 
