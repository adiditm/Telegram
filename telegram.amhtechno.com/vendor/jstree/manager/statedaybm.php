...
if ($vSort =='M')
   ...
   $vOrder.=" order by a.fdebit  ";
			 ...
			 $vsql="select a.*, b.*,c.fnamabank as cfnamabank,  date(a.ftanggal) as ftglcompile from tb_cftrans_sday a left join m_anggota b on a.fidmember=b.fidmember left join m_bank c on b.fnamabank=c.fkodebank where a.ftype='cfsday'"; 
			...
			//Export Excel
				$db->query($vsql);
			//	$vCount=$db->num_rows();
				//if ($vRekMandiri!='') $vRekMandiri = "'".$vRekMandiri;				
				$vArrData=array();
				$vArrHead=array('P',str_replace("-","",$vAkhir),$vRekMandiri,$vCount,$vBonusTot,'','','','','','','','','','','','','');
			   ...
			   $vEmail=$db->f('femail');
				...
				// if ($vSisa >= $vMinTrans)
				   // if ($vRek!='') $vRek = "'".$vRek;	
				//	if ($vCodeClear!='') $vCodeClear = "'".$vCodeClear;	
...
<div class="right_col" role="main">
		<div><label><h3>Daily Transfer (Bank Mandiri)</h3></label></div>
...
<? if ($_SESSION['Priv'] == 'administrator') {?>		  
 <tr>
	...
	?>
        <table border="0" class="table table-striped" style="width:98%">
          <tr style="font-weight:bold">
            ...
            <td style="height: 24px; width: 14%;" align="center"><strong>Nama Penerima<br><span style="color:blue;float:right"><?=str_replace("-","",$vAkhir)?></span></strong></td>
   ...
   for ($i=0;$i<$vPageCount;$i++) {
     ...
     if ($i!=$vPage) {
?>
          <a href="statedaybm.php?uPage=<?=$i?>&amp;uAwal=<?=$oPhpdate->DMY2YMD($oPhpdate->YMD2DMY($vAwal,"-"),"-")?>&amp;uAkhir=<?=$oPhpdate->DMY2YMD($oPhpdate->YMD2DMY($vAkhir,"-"),"-")?>" >
          <?=$i+1?>
          </a>
          <?
  } else {
...
<button class="btn btn-info btn-sm" onClick="document.location.href='../manager/getexcel.php?arr=statedaybm&file=daily_statement_report_<?=$vUser?>'"><i class="fa fa-file-text-o"></i> Export Excel</button> &nbsp;
<button class="btn btn-info btn-sm" onClick="document.location.href='../manager/getcsv.php?arr=statedaybm&file=daily_statement_report_<?=$vUser?>'"><i class="fa fa-file-text-o"></i> Export CSV</button> 
