...
// $vFrom=$oRules->getMailFrom();
 ...
 if ($vAction=="fPost") {
   ...
   if ($vKorek>=0 && $vKorek != $vSaldo) {
		...
		$vDesc="Automain Wallet Direct Change - $vKet";
		...
		if ($vNom < 0) {//kredit
		   ...
		   $oKomisi->insertMutasiAuto($vUserChoosed,$vUser,date("Y-m-d H:i:s"),$vDesc,abs($vNom),0,$vKorek,'koreksiau') ;
		   ...
		   $oKomisi->insertMutasiAuto($vUserChoosed,$vUser,date("Y-m-d H:i:s"),$vDesc,0,abs($vNom),$vKorek,'koreksiau') ;
		...
		$oMember->updateMemField('fsaldowprod',$vUserChoosed, $vKorek) ;
		...
		$_SESSION['referer']='';
		 ...
		 $oSystem->jsAlert("Isikan koreksi saldo Automain dengan benar!");
...
<h1 class="page-header">Koreksi Saldo Automain <?=$vUserChoosed?></small></h1>
