...
echo $vMsgAll.="<html><head><title>Compile &amp; Reset Bonus Stockist Bulanan </title></head><body>";
$vMsg="";
...
//	exit ;
 ...
 //  $vPairFeeSet = $vPairFeeSet / 100; 
   if (true) {   
	   ...
	   while ($dbin->next_record()) {
		...
		$vMsgAll .= $vMsg;
		     ...
		     //$vSQLin="select sum(ffee) as fsum from tb_mutasi where fkind in('titik','uni','profit','reorder','reorderms') and fidmember='$vUser' and date(ftanggal) between date('$vLastMonthStart') and date('$vLastMonthEnd') or (fidmember='$vUser' and date(ftanggal)=date(now()) and fkind in('titik','uni')) ";
  $vSQLin="select sum(ffee) as fsum from tb_kom_stockist where fkind in('bnstrxkitst','bnstrxst','bnsone') and fidmember='$vUser' and date(ftanggal) between date('$vLastMonthStart') and date('$vLastMonthEnd')  ";		    
			
			$db->query($vSQLin);
		    $db->next_record();
	    	echo $vMsg="<br><font color='#0f0'>Hitung bonus stockist bulanan ('bnstrxst','bnstrxkitst','bnsone')  $vLastMonthStart s/d $vLastMonthEnd :" ;
			$vMsgAll .= $vMsg;
		    ...
		    //Masukkan bonus
		    if ($vBonus > 0) {
				echo $vMsg="<font color='#00f'>Deduct bonus stockist bulanan :  $vBonus  </font><br>";
				$vMsgAll .= $vMsg;
				...
				$vUserL=$_SESSION['LoginUser'];
				 $vsql="insert into tb_kom_stockist (fidmember, fidfunder, ftanggal, fdesc, ffee, fdebit, fbalance, fkind, ffeestatus, flastuser, flastupdate,fincometax) "; 
				 $vsql.="values ('$vUser', 'Multiple', '$vDateCompileTime','Withdraw / Reset bonus stockist bulanan (Stockist, Sponsor Stockist, $vLastMonthStart - $vLastMonthEnd)' , 0 ,$vBonus ,$vNewBal ,'resetmonthst' , '1','System' ,'$vDateCompileTime',0) "; 
				
				$vSQLCheck="select * from tb_kom_stockist where fkind='resetmonthst' and date(ftanggal)='$vDateCompile' and fidmember='$vUser' ";
				$db1->query($vSQLCheck);
				$db1->next_record();
				if ($db1->num_rows() <=0) {					
					$db->query($vsql); 
				...
				echo $vMsg="<font color='#f00'><b>Monthly bonus stockist already deducted in this date, no action taken! </b></font><br>";
				$vMsgAll .= $vMsg;					
				}
  ...
  // mail("a_didit_m@yahoo.com","Bonus Pairing Compilation $vNow",$vMsg);
   $vFileName='../files/ResetBonusStockistMonth'.date('Y-m-d_H.i.s').'.htm';
