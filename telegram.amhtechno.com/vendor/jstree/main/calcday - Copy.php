   ...
   if (true) {   
	   ...
	   while ($dbin->next_record()) {
		...
		$vMsgAll .= $vMsg;
		     ...
		     $vSQLin="select sum(fcredit-fdebit) as fsum from tb_mutasi where fkind in('spon','pairing') and fidmember='$vUser' and date(ftanggal) = '$vDate' ";
		    
			
			$db->query($vSQLin);
		    $db->next_record();
	    	echo $vMsg="<font color='#0f0'>Hitung bonus harian (Sponsor + Pasangan) $vDateCompile : " ;
			$vMsgAll .= $vMsg;
		    ...
		    if ($vBonus > 0) {
				...
				echo $vMsg="<font color='#f00'><b>Daily bonus already deducted in this date, no action taken! </b></font><br>";
				$vMsgAll .= $vMsg;					
				}
