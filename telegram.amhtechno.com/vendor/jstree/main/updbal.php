   ...
   if (true) {   
	...
	//    $db->query('START TRANSACTION;');
	   while ($dbin->next_record()) {
         ...
         //echo "<br>";
		 $vUser=$dbin->f('fidmember');
		 $vSQLin="select max(fidsys) as fmaxid from tb_mutasi where  fidmember='$vUser'";
		//echo "<br>";
		 $db1->query($vSQLin);
		 $vCountIn=0;
		 while($db1->next_record()) {
			...
			// echo $vID;
			 ...
			 $vCountIn++;
			 $vSQL3="select * from tb_mutasi where fidsys=$vID";
			//echo "<br>";
			 $dbtime->query($vSQL3);
			 $dbtime->next_record();
			 $vBal=$dbtime->f('fbalance');
			 if ($vBal !='') {
			echo $vSQL2="update m_anggota set fsaldovcr='$vBal' where fidmember='$vUser' "; 
			$dbin1->query($vSQL2);
			echo "<br>";
			 }
