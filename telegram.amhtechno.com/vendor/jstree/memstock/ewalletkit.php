<? 
 ...
 if ($vSearch!="")	{	
  ...
  $vSQL="select count(*) as fjumrec from tb_mutasi_wkit where fidmember='$vUserChoosed' $vCrit";
...
$(document).ready(function(){
  <? if ($oDetect->isMobile()) {?>
  $('#caption').html('<span data-toggle="tooltip" data-placement="top" title="Mutasi Saldo Wallet KIT <?=$oMember->getMemberName($vUserChoosed)?>"><?=substr("Mutasi KIT ".$oMember->getMemberName($vUserChoosed),0,20);?>...</span>');
  <? } else { ?>
	 $('#caption').html('Mutasi Saldo Wallet KIT <?=$vUserChoosed?>');
  <? } ?>
...
<br>
        ...
        <?
			    ...
			    $vSQL="select * from tb_mutasi_wkit where  1 and fidmember='$vUserChoosed' ";
