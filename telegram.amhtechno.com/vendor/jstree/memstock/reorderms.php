  ...
  // print_r($_SESSION['save']);
   ...
   if ($_POST['hPost'] != '1') {
    ...
    while (list($key,$val) = each($_SESSION['save'])) {
    	...
    	$vSQL.=" values('$vNextJual','$vUser','$vBuyer','$vAlamat','$vUser','".$val['lmKode']."',".$val['txtJml'].",now(),".$val['hHarga'].",".$val['hSubTot'].",'".$val['lmSize']."','".$val['lmColor']."',now(),'RO',0,'','','mTrans','Repeat Order',now(),'2','1981-01-01 00:00:00')";
  	 	
  	 	$db->query($vSQL);
  	 	$vTotItem+=$val['txtJml'];
//Stock Position
		$vLastBal = $oMember->getStockPosNex($vUser,$val['lmKode']);
		if ($vLastBal > 0 ){
			...
			$vSQL="update tb_stok_position set fbalance=fbalance-".$val['txtJml']." where fidmember='$vUser' and fidproduk='".$val['lmKode']."';";
			$db->query($vSQL);
		} else {
  ...
  $('#lmCountry').trigger('change');
   ...
   function selectProd(pParam) {
      ...
      $('#thHarga').priceFormat({     
	   ...
	   var vURLQoh='processing_ajax.php?op=getqoh&ms=<?=$vUser?>&idprod='+pParam.value;
	    $('#thQoh').html('<img src="../images/ajax-loader.gif" />');
	   $.get(vURLQoh,function(data){
		   var vQoh=data.trim();
		   if (parseFloat(vQoh) <0)	
		      $('#thQoh').html(vQoh);
			else   $('#thQoh').html(vQoh);
...
function doSaveRow() {
   ...
   if (parseFloat($('#txtJml').val()) <=0 || $('#txtJml').val()=='') {
      alert('Isikan jumlah item, tidak boleh kurang dari atau sama dengan nol!');
      $('#txtJml').focus();
      return false;
   }
   ...
   $.post(vURL, {sernospon : pParam.value},function(data) {
      if (vNo.test(data)) {
         ...
         if (dataX[1]=='nomem')
            $('#statKitSpon').html('<font color="#f00">Member Not Valid!</font>');
             ...
             $('#statKitSpon').html('<font color="#f00">Member Not Valid or not in network!</font>');
      ...
      //  alert(vPhone+':'+vEmail);
         ...
         $.get('../main/mpurpose_ajax.php?op=checkmultiro&user='+pParam.value+'&ymonth='+vYMonth,function(data){
             if(parseFloat(data.trim()) >=999999999999 ) {
...
<? } ?>
  	...
  	<div class="right_col" role="main">
		<div><label><h3>RO Member by Stockist</h3></label></div>
...
<table class="table" >
                            ...
                            <tr>
                                ...
                                <th style="width: 104px; height: 23px;text-align:right" class="hide" >Set Price</th>
                            ...
                            <tr id="trAdd" style="display:">
                                ...
                                <th style="width: 104px; height: 30px;" id="thHarga" class="hide"></th>
                            ...
                            <tr>
                                ...
                                <td style="width: 104px" class="hide">&nbsp;</td>
