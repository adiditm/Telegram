...
$vTom=strtotime('+1 day', strtotime($vLastMonthEnd));
$vDateCompile = date("Y-m-d",$vTom);

 $vCrit="";$vFilterText="";

 if ($vStart!="" || $vEnd!="") {

	    $vCrit.=" and date(ftanggal) >= '$vStartString' and date(ftanggal) <= '$vEndString' or ((date(ftanggal)='$vDateCompile' or date(ftanggal)='$vLastMonthEnd') and fkind='resetmonthst' and fidmember='$vUserChoosed') " ;
	    $vFilterText.="[Date: ".$oPhpdate->YMD2DMY($vStartString)." - ".$oPhpdate->YMD2DMY($vEndString)."]";
		$vSQLCheck = "select * from tb_kom_stockist where fkind='resetmonthst' $vCrit";
		$dbin->query($vSQL);
		
		$vRowsWD=$dbin->num_rows();
}
 ...
 if ($vSearch!="")	{	
   ...
   $vSQL="select count(*) as fjumrec from tb_kom_stockist where fidmember='$vUserChoosed' and fkind in('bnstrxkitst','bnstrxst','bnsone') $vCrit";
...
<div class="right_col" role="main">
		<div><label><h3>Stockist Bonus Statement (Bulanan)</h3></label></div>  
...
<div style="float:left;margin-top:1em;width:100%">
        <p>
			  ...
			  Monthly Bonus Stockist Statement [<?=$oMember->getMemberName($vUserChoosed)?>]
        ...
        <?
			    ...
			    $vSQL="select * from tb_kom_stockist where  1 and fidmember='$vUserChoosed' and fkind in('bnstrxkitst','bnstrxst','bnsone') ";
        ...
        <tr class="tbrekapfont">
          ...
          <td  align="center"><div align="left"><a onClick=" //showDet('<?=$oPhpdate->YMD2DMY($vDateList,"-")?>', '<?=$vDesc?>','<?=number_format($vCred,0,",",".");?>','<?=number_format($vDeb,0,",",".");?>','<?=number_format($vFullNex,0,",",".");?>','<?=number_format($vFullRO,0,",",".");?>','<?=number_format($vFullAll,0,",",".");?>','<?=$vTax?>')" href="#" data-toggle="modalx" data-target="#detailModalx" >
                ...
                <?
                   ...
                   if (!in_array($vKind,array('koreksi','resetmonthst')))
