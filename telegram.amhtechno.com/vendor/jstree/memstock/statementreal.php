...
<?
 ...
 if ($vStart!="" || $vEnd!="") {
	     ...
	     $vCrit.=" and date(ftanggal)  = '$vEndString' and fkind in('resetreal','spon')  AND fidmember='$vUserChoosed ' " ;
...
//echo date("Y-m-d")."ssssssssssss".date_default_timezone_get();
    $vSQL="select count(*) as fjumrec from tb_mutasi where fidmember='$vUserChoosed' and fkind in('resetreal') $vCrit";	
   $db->query($vSQL);
   $db->next_record();
   $vResetCount=$db->f('fjumrec');
   $vSQL="select count(*) as fjumrec from tb_mutasi where fidmember='$vUserChoosed' and fkind in('resetreal','spon') $vCrit";
...
<div class="right_col" role="main">
		<div><label>
		<h3>Realtime Statement Bonus</h3></label></div>  	
...
<br>
        ...
        <?
			  ...
			  //  $vSQL="select * from tb_mutasi where  1 and fidmember='$vUserChoosed' and fkind in('resetweek','spon','pairing') ";
				 $vSQL="select * from tb_mutasi where  1 and fidmember='$vUserChoosed' and fkind in('resetreal','spon') ";
				 $vSQL.=$vCrit;
