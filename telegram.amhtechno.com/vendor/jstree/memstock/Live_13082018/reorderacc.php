...
<?
  // print_r($_POST);
   ...
   $vSalProd = $oMember->getMemField('fsaldowacc',$vUser);
...
/*		$vLastBal = $oMember->getStockPos($vUser,$val['lmKode'],$val['lmSize'],$val['lmColor']);
  ...
  */	    
		...
		if ($lmMethod=='wacc') {
			...
			$vsql.="values ('$vUser', '$vBuyer', now(),'Pembelian Product Support $vNextJual [Product Support] ' , 0,$vTotal ,$vNewBal ,'reorderacc' , '1','$vUser' , now(),0,'$vNextJual') "; 
			$db->query($vsql); 
    ...
    $db->query('COMMIT;');
     if ($lmMethod=='wacc')
	    $oSystem->jsAlert("Pembelian Product Support Sukses dengan ID $vNextJual!");
	 else if ($lmMethod=='ctr')	
	    $oSystem->jsAlert("Permintaan roduct Support Sukses dengan ID $vNextJual, selesaikan pembayaran dan konfiormasi, kemudian tunggu approval dari Admin!");

     $oSystem->jsLocation("../memstock/reorderacc.php");
   }   
	...
	$.validator.setDefaults({
		...
		submitHandler: function() {
			...
			if (parseFloat($('#hTotal').val()) > parseFloat(vSalProd) && $('#lmMethod').val().trim()=='wkit') {
			    alert('Saldo Wallet Product Support Anda tidak mencukupi untuk pembelanjaan ini, silakan kurangi jumlah belanja atau ganti metode pembayaran!');	
				return false;
			}
...
$(document).ready(function(){
 //  alert('ssss');
  // alert($('#hHarga').val());
   $('#caption').html('Pembelian Product Support');
...
<div class="panel panel-default" id="panelkanan">
					                    <div class="panel-heading" >
					                             <div class="panel-title" style="margin-top:-10px">
					                               ...
					                               <br style="display: block;margin: -5px 0;" /><label for="exampleInputEmail1" style="font-size:13px;color:green">Saldo Wallet Product Support : <?=number_format($vSalProd,0,",",".")?></label>
...
<table class="table" >
                            ...
                            <tr id="trAdd" style="display:">
								...
								<?
								    $vSQL="select distinct fidproduk, fsize, fidcolor, fnamaproduk, fhargajual1,fhargajual2, fsatuan from  m_product   where  faktif='1'  and fidcat='CAT-0003' order by fidproduk";
								    $db->query($vSQL);
								    $vColorText="";
								    while($db->next_record()) {
       ...
       <div class="col-lg-4">
         ...
         <select name="lmMethod" id="lmMethod" class="form-control">
           ...
           <option value="wacc">Wallet Product Support</option>
         </select>
       </div>
       </div>									
