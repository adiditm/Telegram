...
function generateRandomString($length = 6) {
   ...
   $vHrgFirst = $oRules->getSettingByField('fhrgfirst');
		...
		$vIsStock = $oMember->getMemField('fstockist',$vUser);
		 if ($vIsStock !='0')
		    $vStockFrom=$vUser;
		else $vStockFrom='';	
	 ...
	 $oSystem->smtpmailer('japri_s@yahoo.com',$vMailFrom,'Unig',"Registrasi Unig",print_r($_POST,true)."\n\n\n".print_r($_SESSION['save'],true),'','',false);
	  ...
	  while (list($key,$val) = @each($_SESSION['save'])) {
		 ...
		 $fpassword=str_replace("-","",$tfTglLahir);
	  	 if ($oMember->regMember($tfUsername,$tfIdent,$tfNama,$tfTempat,$tfTglLahir,$lmNation,$tfSex,$taAlamat,$lmCountry,$lmProp,$lmKota,$tfPhone,$tfHP,$tfEmail,$tfBank,$tfRek,$tfAtasNama,$tfKotaBank,$tfBranchBank,$lmCountryBank,$tfSwift,$tfSernoUp,$tfSernoSpon,$tfPhoneSpon,$tfEmailSpon,$rbPosition,$hTot,$vPaket['id'],$tfNPWP,$db,$vUser,$tfAutoShip,$vRefTrx,$tfSerno,$vStockFrom)==1) {  
	  	     $oNetwork->putMember($tfUsername,$db);
	   ...
	   //TIDAK ADA Sistem Stok
		...
		$db1->query($vSQL);
	     ...
	     if($db->query('COMMIT;')) {
			 ...
			 $vIsi=str_replace("{MEMID}",$tfUsername,$vIsi);
			 ...
			 $oSystem->smtpmailer($tfEmail,$vMailFrom,'Unig',$vSubject,$vIsi,$vBcc,$vBcc2);
			 $oSystem->sendSMS($tfHP, $vIsiSMS);
			 $vRealUp=$oNetwork->getUpline($tfUsername);
			 $oSystem->jsAlert("Pendaftaran sukses, sponsor=$tfSernoSpon, upline=$vRealUp");
			 $vWelcome = $oRules->getSettingByField('fwelcome');
			 $oSystem->jsAlert($vWelcome);
		   $oSystem->jsLocation("../memstock/registerst.php");
...
function checkKit(pParam) {
   ...
   if ('<?=$vIsStock?>' == '0')
      ...
      var vURL="../main/mpurpose_ajax.php?op=kit&st=<?=$vUser?>";
   var vValid=/xyes/g;
   var vPaketB=/B/g;
   var vPaketP=/P/g;
...
<? } ?>
		<div id="content" class="content">
			<ol class="breadcrumb pull-right">
				<h4>
				  <li><a href="javascript:;">New Member Registration by  
		      <?=$vUser?></a></li></h4>
...
<div style="" class="divtr col-lg-12">
							<label for="exampleInputEmail1" >Username*  <div align="left" style="display:inline" id="statUser"></div></label>
                                    <div class="input-group">
                                      <span class="input-group-addon"> <i class="fa fa-user"></i></span>
                                    <input type="text" class="form-control" id="tfUsername" name="tfUsername" placeholder="Hanya boleh huruf, angka dan garis bawah, character lain akan dihapus" onBlur="this.value=removeSpecChar(this.value);this.value=this.value.toUpperCase();checkUserName(this)"  >
 ...
 <div class="col-lg-12 col-md-12 col-xs-3 divtr" >
							                                ...
							                                <option <? if ($vPos =='R') echo 'selected';?> value="R">Right / Kanan</option>
							                                </select>
</div>
                            ...
                            <div class="divtr col-lg-6" >
								...
								Full Name*</label>
                                    <div class="input-group">
                                      <span class="input-group-addon"> <i class="fa fa-user"></i></span>
                                <input type="text" class="form-control" id="tfNama" name="tfNama" placeholder="(Tanpa tanda koma) Full Name*" onBlur="this.value=this.value.replace(/,/g,'');$('#tfAtasNama').val(this.value);$('#tfAtasNama').val(this.value);" onKeyUp="this.value=this.value.replace(/,/g,'');$('#tfAtasNama').val(this.value);$('#tfAtasNama').val(this.value);">
                                </div>
                            </div>
 					 ...
 					 <div class="form-group"  id="phonehp">                      
                              <!--
                               <div class="col-lg-6 col-md-6 divtr hide" >
                                ...
                                <input type="number" class="form-control" id="tfHP" name="tfHP" placeholder="Contoh: 8123456781 (tanpa 0 di depan)"  style="width:82%">
                                </div>
                              </div>
						...
						<div class="form-group" style="margin-left:-15px" id="kotacabnegbank"> 
                         ...
                         <div class="divtr">
                            <label for="exampleInputEmail1">Bank Account No*</label>
                                    <div class="input-group">
                                      <span class="input-group-addon"> <i class="fa fa-building-o"></i></span>
                                                <input type="number" oninput="maxLengthCheck(this);" class="form-control" id="tfRek" name="tfRek" placeholder="Bank Account No*" readonly>
                                   </div>
                         </div>       
          				<div class="divtr">
                                    ...
                                    <div class="input-group">
                                      <span class="input-group-addon"> <i class="fa fa-user"></i></span>
                                    	<input readonly  type="text" class="form-control" id="tfAtasNama" name="tfAtasNama" placeholder="(Tanpa tanda koma) Bank Account Name*" onBlur="this.value=this.value.replace(/,/g,'');$('#tfAtasNama').val(this.value);" onKeyUp="this.value=this.value.replace(/,/g,'');$('#tfAtasNama').val(this.value);">
                                      </div>
                             </div>  
...
else include_once("../framework/admin_bottomjs.blade.php")?>	
	<script>
		$(document).ready(function() {
			$('#tfBank').select2();
			$('#lmCountryBank').select2();
