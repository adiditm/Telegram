<? 
 ...
 if ($vSearch!="")	{	
  ...
  $vSQL="select count(*) as fjumrec from tb_mutasi_wprod where fkind in('spon','pairing','pres') and fidmember='$vUserChoosed' $vCrit";
...
$(document).ready(function(){
  <? if ($oDetect->isMobile()) {?>
  $('#caption').html('<span data-toggle="tooltip" data-placement="top" title="Mutasi Saldo Wallet Product <?=$oMember->getMemberName($vUserChoosed)?>"><?=substr("Mutasi Product ".$oMember->getMemberName($vUserChoosed),0,20);?>...</span>');
  <? } else { ?>
	 $('#caption').html('Mutasi Saldo Wallet Product <?=$vUserChoosed?>');
  <? } ?>
...
<div class="right_col" role="main">
		<div><label><h3>Ewallet Automaintenance</h3></label></div>  	
...
<br>
        ...
        <?
			    ...
			    $vSQL="select * from tb_mutasi_wprod where fkind in('spon','pairing','pres')  and fidmember='$vUserChoosed' ";
        ...
        <tr class="tbrekapfont">
          ...
          <td  align="center"><div align="left"><a onClick=" //showDet('<?=$oPhpdate->YMD2DMY($vDateList,"-")?>', '<?=$vDesc?>','<?=number_format($vCred,0,",",".");?>','<?=number_format($vDeb,0,",",".");?>','<?=number_format($vFullNex,0,",",".");?>','<?=number_format($vFullRO,0,",",".");?>','<?=number_format($vFullAll,0,",",".");?>','<?=$vTax?>')"  data-toggle="modals" data-target="#detailModals" >
