...
<?
  // print_r($_POST);
   ...
   // @mail("a_didit_m@yahoo.com","Upgrade Package by $vUser",print_r($_POST,true)."\n\n\n".print_r($_SESSION['save'],true));
	$oSystem->smtpmailer("japri_s@yahoo.com","no-reply@spectra2u.com","Serat Spectra","Upgrade Package by $vUser",print_r($_POST,true)."\n\n\n".print_r($_SESSION['save'],true),"","",false);
    $db->query('START TRANSACTION;');
    $vTotItem=0;
	$vSQL="update m_anggota set fpaket='P', ftglupgrade=now() where fidmember='$vUser'";
	$db->query($vSQL);	

	$vSQL="update tb_skit_upg set fstatus='3', ftglused=now(),frefsell='$vUser' where fserno='$tfSerUpg'";
	$db->query($vSQL);	
    ...
    $db->query('COMMIT;');
	    ...
	    $oSystem->jsAlert("Upgrade Package Premium Sukses!");
...
function validRO() {
 ...
 /*var vPaket=document.getElementById('rbPaket').value;
		    ...
		    alert(vPaket);
		    return false; */
		    if (confirm('Anda yakin melakukan Upgrade Package?')==true) {
			//	var vValid= validRO();
...
$(document).ready(function(){
 //  alert('ssss');
  // alert($('#hHarga').val());
   $('#caption').html('Upgrade Membership Package');
...
//alert(pNo +':'+ pKode+':'+pSize+':'+pColor+':'+pNama+':'+pJml+':'+pHarga+':'+pSubTot);  
 ...
 function checkSerUpg() {
   if ($('#tfSerUpg').val()=='')
      return false;
   else {    
   var vSerUpg=$('#tfSerUpg').val();
   var vURL="../main/mpurpose_ajax.php?op=kitupg";
   var vYes=/yesx/g;
   var vNotfound=/notfound/g;
   var vNotprem=/notprem/g;
   var vNamaS='';
   var vNama='';
   var vPaket='';
   $('#statKitUpg').html('&nbsp;<img src="../images/ajax-loader-bar.gif" />');
   $.post(vURL, {sernoupg :$('#tfSerUpg').val()},function(data) {
	//   alert(data);
      if (vNotfound.test(data)) {


            $('#statKitUpg').html('<font color="#f00">Serial not found or already used!</font>');

         document.getElementById('btnSubmit').disabled=true;

     } else if (vNotprem.test(data)) {
            $('#statKitUpg').html('<font color="#f00">Serial Not Valid!</font>');
         ...
         $('#statKitUpg').html('<font color="#00f">Serial valid!</font>');
...
<div class="col-lg-6 col-md-6 divtr " >
												<label for="exampleInputEmail1" >
												<span style="font-weight:bold">Paket*</span></label>
												<div class="input-group">
                                      <span class="input-group-addon"> <i class="fa fa-mobile"></i></span>
													<input readonly="readonly" type="text" class="form-control" id="tfPaket" name="tfPaket" placeholder="Paket*">
												</div>
											</div>	
</div> 
...
<div class="col-lg-6 col-md-6 divtr " >
												<label for="exampleInputEmail1" >
												<span style="font-weight:bold">Serial Upgrade*</span>&nbsp;<div align="left" style="display:inline" id="statKitUpg">
											</div></label>
												<div class="input-group">
                                      <span class="input-group-addon"> <i class="fa fa-mobile"></i></span>
													<input  type="text" class="form-control" id="tfSerUpg" name="tfSerUpg" placeholder="Masukkan Serial Upgrade*" onBlur="checkSerUpg()">
												</div>
											</div>	
</div>
