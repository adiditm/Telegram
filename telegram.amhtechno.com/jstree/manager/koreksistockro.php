...
<? include_once("../classes/memberclass.php")?>
 <?php
   ...
   else $_SESSION['referer'] = $vHost."/xsystem/manager/aktif.php?";
 ...
 if ($vAction=="fPost") {
   ...
   if ($vKorek>=0 && $vKorek != $vSaldo) {
		...
		if ($vNom < 0) {//kredit
		   ...
		   $oProduct->insertMutasiStockRO($vUserChoosed,$vProduct,$vSize,$vColor,date("Y-m-d H:i:s"),$vDesc,abs($vNom),0,$vKorek,'adjust') ;
		   ...
		   $oProduct->insertMutasiStockRO($vUserChoosed,$vProduct,$vSize,$vColor,date("Y-m-d H:i:s"),$vDesc,0,abs($vNom),$vKorek,'adjust') ;

		}

		//$pId,$pProd,$pSize,$pColor,$pNom

		$oMember->setSaldoStockNex($vUserChoosed,$vProduct, $vKorek,$db);
		...
		$oSystem->jsLocation("../manager/aktif.php#$vUserChoosed");
		$_SESSION['referer']='';
...
$(document).ready(function(){
      ...
      $('#caption').html('Stock Balance Correction (RO Items)');
...
function checkStock() {
   ...
   //var vURL ='../main/mpurpose_ajax.php?op=checkstock&mem='+vMember+'&prod='+vProd+'&size='+vSize+'&color='+vColor;
   var vURL ='../main/mpurpose_ajax.php?op=checkstockro&mem='+vMember+'&prod='+vProd;
  ...
  <tr valign="top"> 
    ...
    <td align="center" valign="top">
      ...
      <h3>Stock Adjustment for RO Items <?=$oMember->getMemberName($vUserChoosed)?> (<?=$vUserChoosed?>)</h3>      <h4 style="color:blue">Choose Product, Size, and Color, then Check Stock</h4>
...
<select name="lmProduct" id="lmProduct" class="form-control">
                ...
                <? 
                   ...
                   $vSQL="select distinct fidproduk from tb_stok_positionro where fidmember='$vUserChoosed' order by fidproduk";
...
<select name="lmSize" id="lmSize" class="form-control">
                ...
                <? 
                   ...
                   $vSQL="select distinct fsize from tb_stok_positionro where fidmember='$vUserChoosed' order by fsize ";
...
<select name="lmColor" id="lmColor" class="form-control">
                ...
                <? 
                   ...
                   $vSQL="select distinct fcolor from tb_stok_positionro where fidmember='$vUserChoosed' order by fidproduk";
