...
while (list($key,$val) = @each($vcbChoice)) { 
  ...
  $oSystem->jsAlert("$vCount serial upgrade terhapus!");
}
...
if ($_POST['hPostJual']=="1" ){
	...
	if ($oMember->authActiveID($vPembeli)==1 ) {
		...
		while (list($key,$val) = @each($vcbChoice)) { 
		  ...
		  $oSystem->jsAlert("$vCount kartu upgrade terjual kepada \'$vPembeli\'/".$oMember->getMemberName($vPembeli)."!");
...
$(document).ready(function(){
    ...
    $('#caption').html('Stock Serial Upgrade');
...
?>
			...
			?></td>
                  <td width="110" nowrap="nowrap" valign="bottom" align="center" ><input name="cbChoice[]" type="checkbox" id="cbChoice" onclick="document.demoform.cbSelfDel.disabled=!this.checked;document.demoform.cbSelfJual.disabled=!this.checked" value="<?=$vSerno?>" <? if ($vStatus!=3) echo "disabled";?> /></td>
                </tr>
                <? } ?>
              </table>
              </div>
