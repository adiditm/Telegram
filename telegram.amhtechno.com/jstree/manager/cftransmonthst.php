...
if ($vSort =='M')
   ...
   $vOrder.=" order by a.fdebit  ";
			 ...
			 $vsql="select a.*, b.*,c.fnamabank as cfnamabank,  date(a.ftanggal) as ftglcompile from tb_cftransmost a left join m_anggota b on a.fidmember=b.fidmember left join m_bank c on b.fnamabank=c.fkodebank where a.ftype='cfmonth' and ffeestatus='1' and fcftrans>0"; 
			...
			//Export Excel
				$db->query($vsql);
				$vArrData="";
				$vArrHead=array('No.','Date','Username','Name','CF Amount');
				$vArrBlank=array('','','','','');
				...
				// $vNama=$db->f('fnama');
				 ...
				 $vArrData[]=array($i,$vTanggal,$vIdMember,$vNama,$vBelanja);
					//$vArrMem['fpassword'][$i]=$oSystem->doED('decrypt',$db->f('fpassword'));
				
				}
				
				$_SESSION['cftransmonth']=$vArrData;
...
<div class="right_col" role="main">
		<div><label><h3>Monthly CF Trans Bonus Stockist</h3></label></div>
...
<? if ($_SESSION['Priv'] == 'administrator') {?>		  
 <tr>
   ...
   for ($i=0;$i<$vPageCount;$i++) {
     ...
     if ($i!=$vPage) {
?>
          <a href="cftransmonth.php?uPage=<?=$i?>&amp;uAwal=<?=$oPhpdate->DMY2YMD($oPhpdate->YMD2DMY($vAwal,"-"),"-")?>&amp;uAkhir=<?=$oPhpdate->DMY2YMD($oPhpdate->YMD2DMY($vAkhir,"-"),"-")?>" >
          <?=$i+1?>
          </a>
          <?
  } else {
...
<button class="btn btn-info btn-sm" onClick="document.location.href='../manager/getexcel.php?arr=cftransmonthst&file=dmonthly_cftrans_report_<?=$vUser?>'"><i class="fa fa-file-text-o"></i> Export Excel</button> 
