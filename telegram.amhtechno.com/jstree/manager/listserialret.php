...
//	if ($oMember->authActiveID($vPembeli)==1 ) {
		...
		while (list($key,$val) = @each($vcbChoice)) { 
			  ...
			  $vSQL="select * from tb_skit where fserno='$vkd'";
			  $dbin->query($vSQL);
			  $dbin->next_record();
			  $vPembeli=$dbin->f('fpendistribusi');
			  
			  $vsqlin="update tb_skit set fpendistribusi='', ftgldist='1981-01-01 00:00:00',fstatus='1' where fserno='$vkd'";
			  $dbin->query($vsqlin); 
			  
			  $vSQL=" INSERT INTO tb_trxkit( fidpenjualan, fidseller, fidmember, falamatkrm, fnostockist, fserno,ftglentry,fkindtrx) ";			  
			  $vSQL .=" values( '$vIDJual', '$vSeller', '$vPembeli', '', '', '$vkd',now(),'retur') ";		
			  $dbin->query($vSQL); 	  
...
$(document).ready(function(){
    ...
    $('#caption').html('Retur Stock Serial ');
...
<div class="row divtr">
              ...
              <table  border="0" cellspacing="0" cellpadding="0" width="65%" class="table table-striped">
                ...
                <?
  $vsql="select a.*,b.fidmember,b.fnama,a.fpendistribusi from tb_skit a left join m_anggota b on a.fserno=b.fidmember ";
  $vsql.="where 1  $vAnd order by fserno";
