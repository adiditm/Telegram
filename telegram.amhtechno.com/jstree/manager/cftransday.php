...
if ($vSort =='M')
   ...
   $vOrder.=" order by a.fdebit  ";
			 ...
			 $vsql="select a.*, b.*,c.fnamabank as cfnamabank,  date(a.ftanggal) as ftglcompile from tb_cftrans a left join m_anggota b on a.fidmember=b.fidmember left join m_bank c on b.fnamabank=c.fkodebank where a.ftype='cfday' and ffeestatus='1' and fcftrans>0"; 
...
<div class="right_col" role="main">
		<div><label><h3>Daily CF Trans</h3></label></div>
...
<? if ($_SESSION['Priv'] == 'administrator') {?>		  
 <tr>
   ...
   for ($i=0;$i<$vPageCount;$i++) {
     ...
     if ($i!=$vPage) {
?>
          <a href="cftransday.php?uPage=<?=$i?>&amp;uAwal=<?=$oPhpdate->DMY2YMD($oPhpdate->YMD2DMY($vAwal,"-"),"-")?>&amp;uAkhir=<?=$oPhpdate->DMY2YMD($oPhpdate->YMD2DMY($vAkhir,"-"),"-")?>" >
          <?=$i+1?>
          </a>
          <?
  } else {
...
<button class="btn btn-info btn-sm" onClick="document.location.href='../manager/getexcel.php?arr=cftransday&file=daily_cftrans_report_<?=$vUser?>'"><i class="fa fa-file-text-o"></i> Export Excel</button> 
