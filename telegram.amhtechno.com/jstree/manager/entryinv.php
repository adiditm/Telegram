 ...
 //print_r($_SESSION['savestock']);
  ...
  // print_r($_POST);
   ...
   if (count($_POST['KIT_F']) >=1)
    ...
    $db->query('COMMIT;');
     $oSystem->jsAlert("Entry Inventory Sukses dengan ID Transaksi $vNextJual!");

     $oSystem->jsLocation("../manager/entryinv.php");
   }   
   //KIT Only
...
$(document).ready(function(){
 //  alert('ssss');
  // alert($('#hHarga').val());
   $('#caption').html('Entry Inventory Items');
...
function checkKitStockist(pParam) {
   ...
   //alert(vStockist);
      if (parseInt(vStockist)==0) {
         ...
         $('#statKitSpon').html('<font color="#00f">'+pParam.value+' : Member valid (Warehouse)!</font>');
         $('#tfSponsor').val(vNama);
									...
									<div class="panel-body">
										<div class="">
											...
											<div class="iconic-input">
												...
												<input type="text" class="form-control" id="tfSernoSpon" name="tfSernoSpon" placeholder="Member Card/Starter Kit No*" onBlur="checkKitStockist(this)" onKeyUp="setUpper(this)" value="UNEEDS00">
											</div>
										</div>
										<div class="divtr">
...
<table class="table" style="table-layout:fixed;">
							...
							First :<input type="text" id="tfUKitF" name="tfUKitF" class="form-control" style="width:55px;display:inline" value="1" onKeyUp="calcKit('<?=$oRules->getSettingByField('fhrgkit')?>',$('#tfUKitF').val());getAllPurc()" onChange="calcKit('<?=$oRules->getSettingByField('fhrgkit')?>',$('#tfUKitF').val())" />
							 (Total : <span id="sTotKit">300.000</span><input type="hidden" id="hTotKit" name="hTotKit" value="<? echo $oRules->getSettingByField('fhrgkit') * 3;?>">)</div><br>
								 ...
								 <div style="font-size:18px;font-weight:bold">Total 
									 Nilai Inventory : <span id="totBelanja">300.000</span></div>
								<br>
