...
$vSQL="select * from m_anggota where fpaket='P'  order by fidsys ";
$db->query($vSQL);
while($db->next_record()) {
   $vIdMem=$db->f('fidmember');	
   $vOmzetX=$oNetwork->getCountCoupleP($vIdMem);
   
			 $vsql="select * from m_rewards  where 1  order by flevel desc "; 			 
		     $dbin->query($vsql);
			 $vTot=0;
				...
				//  echo  "AAAAAA:$vIdMem:$vOmzetX<br>";
				 if ($vOmzetX >= $vSyaratNom) {
					...
					if ($vRecs <1) {
						...
						$vSQL .=  "value ('$vIdMem', $vLevel, '', $vSyaratNom, $vOmzetX, $vNom, '$vReward', '0', '', '1981-01-01 00:00:00', now())";
						$db1->query($vSQL);


						//SMS
						 $vIsiSMS = $oRules->getSettingByField('fsmsreward');
						 $vHP = $oMember->getNoHP($vIdMem);
						 $vNama = $oMember->getMemberName($vIdMem);
						// "{MEMNAME}, kami ucapkan selamat karena ID {MEMID} mendapat reward {REWARD}";
						 $vIsiSMS=str_replace("{MEMNAME}",$vNama,$vIsiSMS);
						 $vIsiSMS=str_replace("{MEMID}",$vIdMem,$vIsiSMS);
						 $vIsiSMS=str_replace("{REWARD}",$vReward,$vIsiSMS);
						 
						 if ($vHP !='')
							 $oSystem->sendSMS($vHP, $vIsiSMS);
...
if ($vRecordCount > 0) {	
			 ...
			 $vNo++;
				 ...
				 if ($vBukti=='') $vBukti='-';
				 if ($vClaimed=='0') {
				    $vClaimedText='X Belum';
					$vBtnState='';
				 } else	{ $vClaimedText='&radic; Sudah'; 
				 		  $vBtnState='disabled';	
				 }
