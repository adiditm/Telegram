...
// $vFrom=$oRules->getMailFrom();
  ...
  $vSaldo=$oKomisi->getLastBalance($vUserChoosed);
 ...
 if ($vAction=="fPost") {
   ...
   if ($vKorek>=0 && $vKorek != $vSaldo) {
		...
		$vDesc="e-Wallet Cash Direct Change - $vKet";
		...
		if ($vNom < 0) {//kredit
		   ...
		   $oKomisi->insertMutasi($vUserChoosed,$vUser,date("Y-m-d H:i:s"),$vDesc,abs($vNom),0,$vKorek,'koreksi') ;
		   ...
		   $oKomisi->insertMutasi($vUserChoosed,$vUser,date("Y-m-d H:i:s"),$vDesc,0,abs($vNom),$vKorek,'koreksi') ;

		}

		

		$oMember->setSaldo($vUserChoosed, $vKorek);
...
<h1 class="page-header">Koreksi Saldo <?=$vUserChoosed?></small></h1>
