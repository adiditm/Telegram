...
//print_r($_POST); 
         ...
         $vPaketPost=$_POST['hPaket'];
       //  $oSystem->jsAlert("Paket ".$vPaketPost); 
       //  $oSystem->jsAlert(print_r($_SESSION,true));

  	     if (trim($vPaketPost)=='S')
            $vMultiply=$oRules->getSettingByField('fmulsilver');
         else   if (trim($vPaketPost)=='G')
            $vMultiply=$oRules->getSettingByField('fmulgold');   
         else   if (trim($vPaketPost)=='P')
            $vMultiply=$oRules->getSettingByField('fmulplat');  
            
       //  $oSystem->jsAlert("Multi".$vMultiply); 
  ...
  $vAutoShipList= $db->f('fautoship');
   ...
   if ($_POST['hPost'] != '1') {
      ...
      $_SESSION['del']='';
 		...
 		$vSQL="select * from tb_trxstok_member_temp where fidpenjualan='$vRefj'";
		$db->query($vSQL);	
		while($db->next_record()) {
		    $vIDProduk = $db->f('fidproduk');
		    $vAmount= $db->f('fjumlah');
		    $vIdMem=$db->f('fidmember');
			$vCheck=$oJual->checkStock($vUserActive,$vIDProduk,$vAmount);
			if ($vCheck==0) {
			   $oSystem->jsAlert('Salah satu item, stock tidak mencukupi, aktifasi dibatalkan!');
			   $oSystem->jsLocation('../memstock/approval.php');
			   exit;
			}
		}
...
//Kurangi Stock
		...
		while($db->next_record()) {
	        ...
	        $vLastBal=$oMember->getStockPosNex($vUserActive,$vIDProduk);
	        $vNewBal=$vLastBal - $vAmount;
		    
			$vSQL="UPDATE `tb_stok_position`  set fdesc='Sell Stock', fkind='fo', fbalance=fbalance-$vAmount where `fidmember`='$vUserActive' and fidproduk='$vIDProduk' ";
			$db->query($vSQL);
			...
			$vSQL.="VALUES ('$vUserActive', '$vIDProduk' , NULL , NULL , '', now(), 'Member Activation $vIdMem',0, $vAmount, $vNewBal, 'FO', '1', '$vSeller', now(), '$vIdTrx');"; 
			$db->query($vSQL);
			  
		//	$oMember->setSaldoStockNex($vUserActive,$vIDProduk,$vNewBal,$db) ;
   	     ...
   	     $vActiveFee=$oRules->getSettingByField('factivefee');
         $vActiveFee=$vActiveFee * $vMultiply;  
         
   	     $vFeeAdmin=$oRules->getSettingByField('fbyyadmin');
         	
		    $vActiveFeeAdm=($vActiveFee * $vFeeAdmin / 100);
		    $vActiveFeeNett= $vActiveFee- $vActiveFeeAdm;
   	     //Bonus
			$vLastBal=$oMember->getMemField('fsaldovcr',$vUserActive);
			$vNewBal=$vLastBal + $vActiveFee;
			...
			$vsql.="values ('$vUserActive', '$pID', now(),'Bonus aktifasi [$tfSerno]' , $vActiveFeeNett,0 ,$vNewBal ,'aktifasi' , '1','$vUserL' , now(),$vActiveFeeAdm) "; 
			$db->query($vsql); 
			$oMember->updateBalConn($vUserActive,$vNewBal,$db);
...
//alert(pKode);  
	 ...
	 $.get(vURL, function(data) {
	      if (data.trim()=='deleted') {
	         ...
	         document.location.href='../memstock/approval.php';
	      } else {
...
<div class="col-lg-6 col-md-3 col-sm-4 divtr" >
	...
	<div class="col-lg-6 col-md-3 col-sm-4 divtr" >
							                                ...
							                                <input type="hidden" id="hPaket" name="hPaket" value="<?=$db->f('fpaket')?>" >
                            </div>
<!-- ========================= -->
 ...
 <div class="row">
                             <div class="col-md-6 form-group ">
                               ...
                               <div>
  								 ...
  								 <button  type="button" class="btn  btn-primary"  onClick="doPreview()" >
                                <i class="fa fa-print"></i>
                                Print Preview
                                </button>
                                <div class="iconic-input" style="display:inline">
                                    <i class="fa fa-floppy-o"></i>
                                    <input type="button" class="btn  btn-primary" value="Activate" onClick="doActivate('<?=$vUserList?>')" >
  								...
  								<div class="iconic-input" style="display:inline">
                                    ...
                                    <input type="button" class="btn  btn-danger" value="Reject / Delete" onClick="doReject('<?=$vUserList?>')" ><span id="tdLoad"></span>
  								...
  								<div class="iconic-input" style="display:inline">
                                    ...
                                    <input  type="button" class="btn  btn-default" value="Cancel" onClick="document.location.href='../memstock/approval.php'" ><span id="tdLoad"></span>
                                </div>
