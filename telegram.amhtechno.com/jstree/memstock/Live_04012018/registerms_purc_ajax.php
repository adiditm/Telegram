...
;
			      ...
			      echo '<font color="#f00"><b>Kode produk '.$lmKode.' sudah ada dalam list item Anda, silakan hapus dan tambahakan lagi jika ingin mengoreksi Ukuran dan atau Jumlah!</b></font>';
...
<table class="table table-striped" >
                            ...
                            <tr id="trAdd" style="display:">
                                ...
                                <select name="txtJml" id="txtJml" class="form-control" style="display:none;min-width:55px;text-align:right" onChange="calcSub(this)">
                                <option value="0">--Pilih Qty--</option>
                                <? 
								   $vSQL="select distinct fberat from m_product order by fberat";
								   $db->query($vSQL);
								   while ($db->next_record()) {
									   $vBerat=$db->f('fberat');
								?>
                                   <option value="<?=$vBerat?>"><?=$vBerat?></option>	   
								   <? } ?>
								?>
                                </select>                                
                                ...
                                <th style="width: 162px; height: 30px;text-align:right" id="thQoh"><input type="hidden"  id="hQoh" name="hQoh" value=""></th>
