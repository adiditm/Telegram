			...
			if ($valser !='') {
			   ...
			   // @mail("a_didit_m@yahoo.com","Entri RO Spectra by $vUser",print_r($_POST,true)."\n\n\n".print_r($_SESSION['save'],true));
				...
				while (list($key,$val) = each($_SESSION['save'])) {
					...
					$vSQL.=" values('$vNextJual','$vUser','$valser','$vAlamat','$vUser','".$val['lmKode']."',".$val['txtJml'].",now(),".$val['hHarga'].",".$val['hSubTot'].",now(),'KA',0,'','','KIT Activation',now(),'2','0000-00-00 00:00:00')";
...
<table class="table table-striped" >
                            ...
                            <tr id="trAdd" style="display:">
								...
								<?
								    $vSQL="select distinct fidproduk, fsize, fidcolor, fnamaproduk, fhargajual1,fhargajual2, fsatuan from  m_product   where  faktif='1' and fidproduk in (select fidproduk from tb_stok_position where fidmember='$vUserHO')  order by fidproduk";
								    $db->query($vSQL);
								    $vColorText="";
								    while($db->next_record()) {
