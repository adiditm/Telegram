  ...
  // print_r($_SESSION['save']);
   ...
   if ($_POST['hPost'] != '1') {
    ...
    while (list($key,$val) = each($_SESSION['save'])) {
    	...
    	$vSQL.=" values('$vNextJual','$vUser','$vBuyer','$vAlamat','$vOutLet','".$val['lmKode']."',".$val['txtJml'].",now(),".$val['hHarga'].",".$val['hSubTot'].",'".$val['lmSize']."','".$val['lmColor']."',now(),'RO',0,'','','mTrans','Repeat Order',now(),'2','0000-00-00 00:00:00',0,$tfShCost,$tfDiscG)";
