  ...
  $vHrgKit = $oRules->getSettingByField('fhrgkit');    
   //Purchase
	if ($_POST['tfUKitE'] > 0) {        
    	...
    	$vSQL.=" values('$vNextJual','$vUserHO','$vBuyer','$vAlamat','$vUser','',$vJmlKitEx,now(),$vHrgKit,$vSubTot,'S','mTrans','KIT Order',now(),'0','1981-01-01 00:00:00','kitorder')";
  	 	$db->query($vSQL);
    }
	...
	if ($_POST['tfUKitB'] > 0) {        
        $vSubTot = $vHrgKit * $vJmlKitEc;    		
       	$vSQL="insert into tb_trxkit(fidpenjualan,fidseller,  fidmember, falamatkrm, fnostockist, fserno, fjumlah, ftanggal, fhargasat, fsubtotal,  fjenis,  fmethod, fketerangan, ftglentry, fprocessed, ftglprocessed,fkindtrx)";
    	$vSQL.=" values('$vNextJual','$vUserHO','$vBuyer','$vAlamat','$vUser','',$vJmlKitEc,now(),$vHrgKit,$vSubTot,'G','mTrans','KIT Order',now(),'0','1981-01-01 00:00:00','kitorder')";
  	 	$db->query($vSQL);
    }	
	...
	if ($_POST['tfUKitF'] > 0) {        
        $vSubTot = $vHrgKit * $vJmlKitEt;    		
       	$vSQL="insert into tb_trxkit(fidpenjualan,fidseller,  fidmember, falamatkrm, fnostockist, fserno, fjumlah, ftanggal, fhargasat, fsubtotal,  fjenis,  fmethod, fketerangan, ftglentry, fprocessed, ftglprocessed,fkindtrx)";
    	$vSQL.=" values('$vNextJual','$vUserHO','$vBuyer','$vAlamat','$vUser','',$vJmlKitEt,now(),$vHrgKit,$vSubTot,'P','mTrans','KIT Order',now(),'0','1981-01-01 00:00:00','kitorder')";
  	 	$db->query($vSQL);
    }			
    $db->query('COMMIT;');
...
<div class="right_col" role="main">
		<div><label><h3>Request PO for Serial</h3></label></div>
        <!-- page start-->
