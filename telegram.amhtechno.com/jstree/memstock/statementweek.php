  ...
  $vSpy = md5('spy').md5($_GET['uMemberId']);
   if ($_GET['op']==$vSpy)
	...
	function getStartAndEndDate($week, $year) {
     ...
     $vWeekRange = getStartAndEndDate($vWeek,$vYear);
//	 print_r($vWeekRange);
  ...
  if (isset($vRefUser))
  	 ...
  	 $vUserChoosed=$_SESSION['LoginUser'];
		$vNow = strtotime("today");
		$vYest = strtotime("-1 day",$vNow);	
 ...
 if ($vStart=='') { 
	 $vStart= $vWeekRange['week_start'];	
 }
	
 if ($vEnd=='') { 
	  $vEnd= $vWeekRange['week_end'];	
 }
 
  //  $vStart=date("Y-m-d",strtotime("-1 day",strtotime($vEnd)));
 ...
 if ($vStart!="" || $vEnd!="") {
	     ...
	     $vCrit.=" and date(ftanggal) >= '$vStartString' and date(ftanggal) <= '$vEndString' " ;
		 $vCritReset = " and  date(ftanggal) = DATE_ADD('$vEnd', INTERVAL 1 DAY);";
		 $vCritResetOr = "  or  (date(ftanggal) = DATE_ADD('$vEnd', INTERVAL 1 DAY)  and fidmember = '$vUserChoosed') ";
...
//echo date("Y-m-d")."ssssssssssss".date_default_timezone_get();
    $vSQL="select count(*) as fjumrec from tb_mutasi where fidmember='$vUserChoosed' and fkind in('resetweek') $vCritReset";	
   $db->query($vSQL);
   $db->next_record();
   $vResetCount=$db->f('fjumrec');
   $vSQL="select count(*) as fjumrec from tb_mutasi where fidmember='$vUserChoosed' and fkind in('resetweek','pres') $vCrit";
...
$(document).ready(function(){
  <? if ($oDetect->isMobile()) {?>
  $('#caption').html('<span data-toggle="tooltip" data-placement="top" title="Daily Bonus Statement <?=$oMember->getMemberName($vUserChoosed)?>"><?=substr("Daily  Statement  ".$oMember->getMemberName($vUserChoosed),0,20);?>...</span>');
  <? } else { ?>
	$('#caption').html('Daily Bonus Statement <?=$vUserChoosed?>');
  <? } ?>
...
function changeWeek(pWeek,pYear) {
	var vURL = "../main/mpurpose_ajax.php?op=getrweek&week="+pWeek+"&year="+pYear;
	$.get(vURL,function(data){
		  var vObj = $.parseJSON(data);
		  $('#dc').val(vObj.week_start);
		  $('#dc1').val(vObj.week_end);
	});
}
</script>
...
<div class="right_col" role="main">
		<div><label><h3>Weekly Statement Bonus</h3></label></div>  	
...
<div class="row">
            <div align="left" class="col-lg-1"><strong>Week </strong></div>
             <div align="left" class="col-lg-1">
              <select id="lmWeek" name="lmWeek" class="form-control" onchange="changeWeek(this.value,document.getElementById('lmYear').value)">
                   <? for ($i=0;$i<52;$i++) {?>
                        <option value="<?=$i+1?>"  <? if($vWeek==($i+1)) echo "selected";?> ><?=$i+1?></option>
                   <? } ?>
              </select>
             </div>
			  <div align="left" class="col-lg-1"><strong>Year </strong></div>
             <div align="left" class="col-lg-2">
              <select id="lmYear" name="lmYear" class="form-control" onchange="changeWeek(document.getElementById('lmWeek').value, this.value)">
                   <? 
				    $vYNow=date("Y");
				    for ($i=2;$i>0;$i--) {?>
                        <option value="<?=($vYNow-$i)?>"  <? if($vYear==(($vYNow-$i))) echo "selected";?> ><?=($vYNow-$i)?></option>
                   <? } ?>
<?
				    for ($i=0;$i<2;$i++) {?>
                        <option value="<?=($vYNow+$i)?>"  <? if($vYear==(($vYNow+$i))) echo "selected";?> ><?=($vYNow+$i)?></option>
...
<div class="row">
            <div align="left" class="col-lg-1"><strong>Date </strong></div>

<span class="">
              <div  class="col-lg-2  col-sm-2 col-md-2 col-xs-4 " style="min-width:10em">
              <input class="dpick form-control form-inline"  name="dc"  id="dc"  value="<?=$vStart?>"  readonly="readonly"  />
              </div>
              
              <div class="col-lg-1 col-md-1 col-sm-1 col-xs-1"><strong>To</strong></div> </span>
                            
              <div  class="col-lg-2 col-sm-2 col-md-2 col-xs-4" style="min-width:10em">
              <input  class="dpick form-control form-inline" name="dc1" id="dc1" value="<?=$vEnd?>" readonly="readonly" /></div>
...
<br>
        ...
        <?
			  ...
			  //  $vSQL="select * from tb_mutasi where  1 and fidmember='$vUserChoosed' and fkind in('resetweek','spon','pairing') ";
				 $vSQL="select * from tb_mutasi where  1 and fidmember='$vUserChoosed' and fkind in('resetweek','pres') ";
			 	$vSQL.=$vCrit." ".$vCritResetOr;
				...
				while ($db->next_record()) {
				   ...
				   $vDeb=$db->f("fdebit");
				   $vBal=$db->f("fbalance");
				   $vTax=$db->f("fincometax");
				   if ($vCred >0)
				   $vByy=$vTax / ($vTax + $vCred) * 100;
				   
				   if (in_array($vKind,array('resetweek','pres'))) {
        ...
        <tr class="tbrekapfont">
          ...
          <td  align="center"><div align="left"><a onClick=" //showDet('<?=$oPhpdate->YMD2DMY($vDateList,"-")?>', '<?=$vDesc?>','<?=number_format($vCred,0,",",".");?>','<?=number_format($vDeb,0,",",".");?>','<?=number_format($vFullNex,0,",",".");?>','<?=number_format($vFullRO,0,",",".");?>','<?=number_format($vFullAll,0,",",".");?>','<?=$vTax?>')" href="#" data-toggle="modalx" data-target="#detailModalx" >
                ...
                <?
                   ...
                   if (!in_array($vKind,array('koreksi','resetweek')))
...
<?=number_format($vBal,0,",",".");?>
  ...
  <button style="margin-left:2em" class="btn btn-info btn-sm " onClick="document.location.href='../manager/getexcel.php?arr=weekstate&file=daily_bonus_<?=$vUserChoosed?>'"><i class="fa fa-file-text-o"></i> Export Excel</button> 
  <button style="margin-left:2em" class="btn btn-info btn-sm hide" onClick="document.location.href='../manager/getexcel.php?arr=weekstate&file=weekly_bonus_<?=$vUserChoosed?>'"><i class="fa fa-file-text-o"></i> Print</button> 
  <?
  																														 //function showDetail(pTgl,pDesc,pCred,pDeb,pFullNex,pFullRO,pFullAll,pTax)
  ?>
  <button  data-toggle="modal" data-target="#detailModal" style="margin-left:2em" class="btn btn-success btn-sm hide" onClick="showDetail('<?=$vFilterText?>','Weekly Bonus&nbsp;','<?=number_format($vTotBon,0,",",".")?>','0','<?=number_format($vFullBonus,0,",",".")?>','<?=number_format($vFullRO,0,",",".")?>','<?=number_format($vFullAll,0,",",".")?>','<?=$vTax?>')"><i class="fa fa-bars"></i> Detail Statement</button> 
