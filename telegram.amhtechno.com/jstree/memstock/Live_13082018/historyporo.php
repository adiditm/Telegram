...
$(document).ready(function(){
    $('#caption').html('Purchase Order History (RO Items) <?=$vUserActive?> (<?=$oMember->getMemberName($vUserActive);?>)');
...
<br />
    ...
    <div class="table-responsive">
        <table width="90%" border="0" class="table table-striped">
          ...
          <? 
             $vNo=0;
			 $vsql="select distinct ftanggal, fidpenjualan,fidseller,fidmember, fketerangan, '1' as fstatus  from tb_trxstok where  fidmember='$vUserActive' and fjenis='PSRO'"; 
			 $vsql.=$vCrit;

			 
			 $vsql.=" union all select distinct ftanggal, fidpenjualan,fidseller,fidmember, fketerangan, '0' as fstatus  from tb_trxstok_temp where  fidmember='$vUserActive' and fjenis='PSRO'"; 
			 $vsql.=$vCrit;
