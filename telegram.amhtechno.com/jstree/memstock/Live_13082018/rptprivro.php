...
<?
 ...
 if ($vSearch!="")	{	
  ...
  $vSQL="select count(*) as fjumrec from tb_mutasi where fidmember='$vUserChoosed' and fkind in('bonusro') $vCrit";
...
$(document).ready(function(){
  ...
  <? if ($oDetect->isMobile()) {?>
  $('#caption').html('<span data-toggle="tooltip" data-placement="top" title="Bonus Belanja Pribadi <?=$oMember->getMemberName($vUserChoosed)?>"><?=substr("Bns Belanja Pribadi ".$oMember->getMemberName($vUserChoosed),0,20);?>...</span>');
  <? } else { ?>
	$('#caption').html('Bonus Belanja Pribadi <?=$vUserChoosed?>');
  <? } ?>
...
<br>
        ...
        <?
			  ...
			  //  $vSQL="select * from tb_mutasi where  1 and fidmember='$vUserChoosed' and fkind in('resetweek','spon','pairing') ";
				 $vSQL="select * from tb_mutasi where  1 and fidmember='$vUserChoosed' and fkind in('bonusro') ";
				$vSQL.=$vCrit;
