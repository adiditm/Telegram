<? 
 ...
 if ($vSearch!="")	{	
  ...
  $vSQL="select count(*) as fjumrec from tb_mutasi_wacc where fidmember='$vUserChoosed' $vCrit";
...
$(document).ready(function(){
  <? if ($oDetect->isMobile()) {?>
  $('#caption').html('<span data-toggle="tooltip" data-placement="top" title="Mutasi Saldo Support <?=$oMember->getMemberName($vUserChoosed)?>"><?=substr("Mutasi Support ".$oMember->getMemberName($vUserChoosed),0,20);?>...</span>');
  <? } else { ?>
	 $('#caption').html('Mutasi Saldo Support <?=$vUserChoosed?>');
  <? } ?>
...
<br>
        ...
        <?
			    ...
			    $vSQL="select * from tb_mutasi_wacc where  1 and fidmember='$vUserChoosed' ";
