...
<?
 ...
 if ($vSearch!="")	{	
  ...
  $vSQL="select count(*) as fjumrec from tb_mutasi where fidmember='$vUserChoosed' and fkind in('bonusmega') $vCrit";
...
$(document).ready(function(){
  ...
  <? if ($oDetect->isMobile()) {?>
  $('#caption').html('<span data-toggle="tooltip" data-placement="top" title="Bonus Mega Matching <?=$oMember->getMemberName($vUserChoosed)?>"><?=substr("Bns Mega Matching ".$oMember->getMemberName($vUserChoosed),0,20);?>...</span>');
  <? } else { ?>
	 $('#caption').html('Bonus Mega Matching  <?=$vUserChoosed?>');
  <? } ?>
...
<br>
        ...
        <?
			  ...
			  //  $vSQL="select * from tb_mutasi where  1 and fidmember='$vUserChoosed' and fkind in('resetweek','spon','pairing') ";
				 $vSQL="select * from tb_mutasi where  1 and fidmember='$vUserChoosed' and fkind in('bonusmega') ";
				$vSQL.=$vCrit;
